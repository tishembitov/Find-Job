...
def select_all_from_vacancy(connector: StoreConnector):
    connector.start_transaction()  # начинаем выполнение запросов
    query = f'SELECT * FROM vacancy ORDER BY name'
...
def insert_rows_into_vacancy_data(connector: StoreConnector, dataframe: DataFrame):
    ...
    connector.start_transaction()
    for row in rows:
        connector.execute(f'INSERT INTO vacancy (name, employer, status, shedule, experience, salary, skills, city, street, building, logo) VALUES (\'{row["vacancy_name"]}\', \'{row["employer_name"]}\', \'{row["vacancy_type"]}\', \'{row["vacancy_schedule"]}\', \'{row["vacancy_experience"]}\', \'{row["vacancy_salary"]}\', \'{row["skill_name"]}\', \'{row["city"]}\', \'{row["street"]}\', \'{row["building"]}\',  \'{row["logo"]}\')')
    connector.end_transaction()
...
def insert_rows_into_mydocuments_data(connector: StoreConnector):
    connector.start_transaction()
    connector.execute(f'INSERT INTO mydocuments (user_id) VALUES (1)')
    connector.end_transaction()

def insert_rows_into_myresumes_data(connector: StoreConnector):
    connector.start_transaction()
    connector.execute(f'INSERT INTO myresumes (mydocuments_id, resume_id) VALUES (1, 1)')
    connector.end_transaction()

def insert_rows_into_myvacancies_data(connector: StoreConnector):
    connector.start_transaction()
    connector.execute(f'INSERT INTO myvacancies (mydocuments_id, vacancy_id) VALUES (1, 1)')
    connector.end_transaction()
...
def delete_from_source_files(connector: StoreConnector):
    connector.start_transaction()
    query = f'DELETE FROM source_files WHERE id>10'
    result = connector.execute(query)
    connector.end_transaction()
    return result
...
def delete_from_vacancy(connector: StoreConnector):
    connector.start_transaction()
    query = f'DELETE FROM vacancy WHERE id>10'
    result = connector.execute(query)
    connector.end_transaction()
    return result
...
def delete_from_user(connector: StoreConnector):
    connector.start_transaction()
    query = f'DELETE FROM user WHERE id>10'
    result = connector.execute(query)
    connector.end_transaction()
    return result
...
def delete_from_resume(connector: StoreConnector):
    connector.start_transaction()
    query = f'DELETE FROM resume WHERE id>20'
    result = connector.execute(query)
    connector.end_transaction()
    return result
